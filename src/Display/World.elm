...
groundMesh =
    let green = color (degrees 110) 0.48
        topLeft     = Vertex (vec3 -1  1 0) (green 0.7)
        topRight    = Vertex (vec3  1  1 0) (green 0.4)
        bottomLeft  = Vertex (vec3 -1 -1 0) (green 0.5)
        bottomRight = Vertex (vec3  1 -1 0) (green 0.6)
        t = (makeRotate (pi/2) i) `mul` makeScale (vec3 10 10 0)
    in
        map (mapTriangle (\v -> { v | position <- transform t v.position }))
